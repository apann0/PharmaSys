<?php
/**
 * Add Medicine – Form to create a new medicine entry.
 */
require_once __DIR__ . '/../../includes/auth_check.php';

$pageTitle  = 'Add Medicine';
$activePage = 'add_medicine';
$basePath   = '../../';
...
$errors = [];
$old = ['name'=>'','category_id'=>'','stock'=>'','price'=>'','purchase_price'=>'','expiry_date'=>'','description'=>''];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($old['expiry_date'] && !strtotime($old['expiry_date'])) $errors[] = 'Invalid expiry date.';

    // Check duplicate name
    if (empty($errors)) {
        $chk = $pdo->prepare('SELECT COUNT(*) FROM medicines WHERE name = ?');
        $chk->execute([$old['name']]);
        if ($chk->fetchColumn() > 0) $errors[] = 'A medicine with this name already exists.';
    }

    if (empty($errors)) {
        $stmt = $pdo->prepare('INSERT INTO medicines (name, category_id, stock, price, purchase_price, expiry_date, description) VALUES (?,?,?,?,?,?,?)');
        $stmt->execute([
            $old['name'],
        ...
        logAction($_SESSION['user_id'], 'ADD_MEDICINE', "Added medicine: {$old['name']}");
        setFlash('success', "Medicine \"{$old['name']}\" added successfully.");
...
<div class="glass" style="padding:1.5rem;max-width:720px;">
  <h3 style="font-size:1rem;font-weight:700;margin-bottom:1.25rem;">New Medicine</h3>
  ...
  <form method="POST">
    ...
    <div class="form-grid">
      <div class="form-group">
        ...
        <input type="text" id="name" name="name" class="form-input" required
               value="<?php echo e($old['name']); ?>" placeholder="e.g. Paracetamol 500mg">
      </div>
      ...
      <div class="form-group">
        ...
        <input type="number" id="purchase_price" name="purchase_price" class="form-input" min="0" step="0.01"
    ...
    <div class="form-group">
      ...
      <textarea id="description" name="description" class="form-textarea"
                placeholder="Optional notes…"><?php echo e($old['description']); ?></textarea>
    </div>
